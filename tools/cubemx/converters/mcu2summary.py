...
def __getPins(pinNodes: list[etree.Element], ns: dict) -> dict:
    pins = {}
    segs = []
    for pinNode in pinNodes:
        ...
        for signalNode in signalNodes:
            ...
            modes = signalNode.attrib.get('IOModes', None)
            if modes is not None:
                modes = modes.split(',')
                for mode in modes:
                    signals[f'{signalName}-{mode}'] = {'mode': f'stmicroelectronics.{mode.lower()}'}
            else:
                signals[signalName] = {}

        pinItem = {'position': position, 'type': type_, 'signals': signals}
        if pinName in pins:
    ...
    for seg in segs:
        ...
        for i in range(num):
            pins[f'{seg}-{i + 1}'] = ps[i]
    # noinspection PyUnresolvedReferences
    _pins = sorted(pins.items(), key=__sortByPositionKey, reverse=False)
    pins = {k: v for k, v in _pins}
    return pins


def __getModules(ipNodes: list[etree.Element], ns: dict) -> dict:
    ips = {}
    for ipNode in ipNodes:
        name = ipNode.attrib['Name']
        instanceName = ipNode.attrib['InstanceName']
        group = 'Misc'
        if name in __modules_group:
            group = __modules_group[name]['group']
        else:
            pass
...
def __getDocs(name: str, cursor: sqlite3.Connection) -> dict:
    ...
    for rpnHasResource in rpnHasResources:
        ...
        subcategoryName = __documents_group.get(subcategoryRef, subcategoryRef)
...
def __getNames(name: str, cursor: sqlite3.Connection) -> dict[str, list[str]]:
    rtn = {}
    names = []
    pattern = r'\(.*?\)'
...
def __getUrl(name: str, cursor: sqlite3.Connection) -> str:
...
def __getIntroduction(name: str, cursor: sqlite3.Connection) -> str:
...
def mcu2summary(path: str, crdbPath: str, cfdbPath: str) -> dict:
    rtn = {}
    ...
    pins = __getPins(pinNodes, namespace)
    ipNodes = root.findall('ns:IP', namespace)
    modules = __getModules(ipNodes, namespace)
    names = __getNames(refName, cfdbCursor)
    for namex, ns in names.items():
        url = __getUrl(namex, crdbCursor)
        introduction = __getIntroduction(namex, crdbCursor)
        documents = __getDocs(namex, cfdbCursor)
        for n in ns:
            rtn[n] = {
                'name': n,
                'clockTree': clockTree,
...
def __main():
    ...
    mcus = mcu2summary(src, crdb, cfdb)

    for name_, mcu in mcus.items():
        print(f"find {name_} mcu, {len(mcu['pins'])} pin! write to {Path(dest).absolute()}/{name_.lower()}.yml")
        data = yaml.dump(mcu, sort_keys=False)
        with open(f"{os.path.join(dest, f'{name_.lower()}.yml')}", 'w') as f:
            f.write(data)
