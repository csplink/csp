...
void TestCaseProjectTable::loadProject()
{
    ProjectTable::ProjectType project;
    ProjectTable::loadProject(&project, ":/tests/components/project/project.json");
    QVERIFY(!project.Name.isEmpty());
}

void TestCaseProjectTable::saveProject()
{
...
void TestCaseProjectTable::dumpProject()
{
    auto p = ProjectTable::ProjectType();
    p.Name = "test";
