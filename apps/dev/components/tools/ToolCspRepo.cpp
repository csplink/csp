...
{
    const QString scriptFile = QString("%1/csp-repo/csp-repo.py").arg(Settings.tools());
    const QString data = Python::cmd(scriptFile, { QString("--dump=%1").arg(name), QString("--repository=") + Settings.repository() });
    return data;
}
...
int ToolCspRepo::installPackage(const QString &name, const QString &version)
{
    ...
    const QString scriptFile = QString("%1/csp-repo/csp-repo.py").arg(Settings.tools());
    const int err = python->execv({ scriptFile, QString("--install=%1").arg(name), QString("--package-version=%1").arg(version),
...
int ToolCspRepo::updatePackage(const QString &name)
{
    PythonAsync *python = PythonAsync::getInstance();
    const QString scriptFile = QString("%1/csp-repo/csp-repo.py").arg(Settings.tools());
    const int err = python->execv({ scriptFile, QString("--update=%1").arg(name), QString("--repository=%1").arg(Settings.repository()) });
    return err;
}
...
int ToolCspRepo::uninstallPackage(const QString &name, const QString &version)
{
    PythonAsync *python = PythonAsync::getInstance();
    const QString scriptFile = QString("%1/csp-repo/csp-repo.py").arg(Settings.tools());
    const int err = python->execv({ scriptFile, QString("--uninstall=%1").arg(name), QString("--package-version=%1").arg(version),
                                    QString("--repository=%1").arg(Settings.repository()) });
    return err;
}
