...
int PythonAsync::execv(const QStringList &argv, const QString &workDir)
{
    ...
    const QString program = Settings.python();
    const QMap<QString, QString> env = Settings.env();
    ...
    connect(process, &QProcess::started, this, [process, this]() { emit signalStarted(process); });
