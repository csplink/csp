...
class Coder(QObject):
    ...
    def deploy(self):
        ...
        data = self.__generator.deploy(copy.deepcopy(PROJECT.project().origin), os.path.dirname(PROJECT.path()))
        return data
    ...
    def __loadGenerator(self) -> ModuleType | None:
        if self.__checkHalFolder():
            spec = importlib.util.spec_from_file_location('coder',
                                                          f'{PROJECT.halFolder()}/tools/generator/generator.py')
            module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(module)
            return module
        return None
    ...
    def __render(self, path: str, info: dict, env: jinja2.Environment, args: dict) -> str | None:
...
class CoderCmd(QObject):
    ...
    def __on_coder_generated(self, path: str, index: int, count: int, success: bool):
        if self.progress:
            ...
            if success:
                print(f'generate {path!r}, because the file or config has been modified.')
            else:
