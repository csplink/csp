...
class SummaryTest(unittest.TestCase):
    ...
    def test_getSummary(self):
        ...
        for soc in socs:
            ...
            self.assertGreater(len(summary.vendorUrl), 0, msg='load failed.')
            ...
            for _, doc in documents.datasheets.items():
                self.assertGreater(len(doc.url), 0, msg='load failed.')
            ...
            self.assertGreater(len(summary.illustrate), 0, msg='load failed.')
            self.assertGreater(len(summary.introduction), 0, msg='load failed.')
            ...
            for _, group in modules.items():
                ...
                for _, unit in group.items():
                    self.assertGreater(len(unit.description), 0, msg='load failed.')
                    # self.assertGreater(len(unit.ip), 0, msg='load failed.')
            self.assertGreater(len(summary.package), 0, msg='load failed.')
            self.assertGreater(len(summary.url), 0, msg='load failed.')
            ...
            self.assertGreater(len(linker.defaultHeapSize), 0, msg='load failed.')
            self.assertGreater(len(linker.defaultStackSize), 0, msg='load failed.')
            self.assertGreater(len(summary.pinIp), 0, msg='load failed.')
